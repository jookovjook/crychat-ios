...
func sendMessage(_ message: String, to : String, completionHandler: @escaping (Bool) -> ()){
    ...
    print("String before conversion")
    print("----")
    print(encryptedData)
    let encryptedMessage = encryptedData.encrypt(with: pubKey(from: pub_Key)!)
    let key1 = convertToString(["key":priv_Key]).encrypt( with: pubKey(from: publicKey)!)
    let key2 = convertToString(["key":priv_Key]).encrypt( with: pubKey(from: to)!)
